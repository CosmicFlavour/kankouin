...
#[test]
fn import_swaps_the_live_connection_to_imported_data() {
    let dir = tempfile::tempdir().unwrap();
    let source_db = dir.path().join("source.sqlite3");
    let target_db = dir.path().join("target.sqlite3");
    let export_file = dir.path().join("export.kankouin");

    let source_conn = db::open_and_migrate(&source_db).unwrap();
    source_conn.execute(WORKSPACE_INSERT, []).unwrap();
    kankouin_lib::commands::sync::export_to_file(&source_conn, "pw", &export_file).unwrap();
    drop(source_conn);

    let mut target_conn = db::open_and_migrate(&target_db).unwrap();
    target_conn
        .execute(
            "INSERT INTO workspaces (id, name, created_at, updated_at) \
             VALUES ('local', 'Local Only', '2026-01-01T00:00:00Z', '2026-01-01T00:00:00Z')",
    ...
    kankouin_lib::commands::sync::import_swapping_connection(&mut target_conn, "pw", &export_file)
        .unwrap();

    // The same, still-open Connection object must now see the imported data
    // (and only the imported data) — proving the *live* connection was
    // swapped, not just the file on disk.
...
#[test]
fn import_swap_failure_restores_original_connection_and_data() {
    let dir = tempfile::tempdir().unwrap();
    let target_db = dir.path().join("target.sqlite3");
    let bogus_file = dir.path().join("bogus.kankouin");
    fs::write(&bogus_file, b"not a real export").unwrap();
    ...
    let mut target_conn = db::open_and_migrate(&target_db).unwrap();
    target_conn.execute(WORKSPACE_INSERT, []).unwrap();

    let result = kankouin_lib::commands::sync::import_swapping_connection(
        &mut target_conn,
        "any passphrase",
        &bogus_file,
    );
    ...
    // The connection must be usable and see the original data — not left
    // pointing at the throwaway in-memory placeholder used mid-swap.
    let name: String = target_conn
        .query_row("SELECT name FROM workspaces WHERE id = 'ws1'", [], |row| {
            row.get(0)
        })
